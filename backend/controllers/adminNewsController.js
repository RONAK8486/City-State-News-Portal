...
// @desc    Get 5-day wise report
// @route   GET /api/admin/stats/5-days
// @access  Private/Admin
exports.get5DayReport = async (req, res) => {
  try {
    const fiveDaysAgo = new Date();
    fiveDaysAgo.setDate(fiveDaysAgo.getDate() - 4); // 5 days including today
    fiveDaysAgo.setHours(0, 0, 0, 0);

    const report = await News.aggregate([
      {
        $match: {
    ...
    for (let i = 0; i < 5; i++) {
        let d = new Date();
        d.setDate(d.getDate() - 4 + i); // 4 days ago to today
        let dateStr = d.toISOString().split('T')[0];
        
        let found = report.find(r => r._id === dateStr);
        if (found) {
            result.push(found);
        } else {
            result.push({ _id: dateStr, totalCreated: 0, totalViews: 0, totalLikes: 0 });
        }
    }
